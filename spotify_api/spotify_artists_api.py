...
try:
    ...
    for remove in spotify_json:
        ...
        for name in spotify_json['items']:
            ...
            # album_lst.append({album_name: album_pic})
            album_dict.update({
    ...
    for song, pic in no_dupe_album_dict.items():
        print(F.CYAN + song + F.WHITE + ":", pic + F.RESET)
        # print(song, "is being saved..")
        # urllib.request.urlretrieve(pic, f"./apis_python/spotify_api/gunna_albums/{song}.jpg")
        print()

    # for view in album_lst:
    #     print(view)

    # for i in album_lst:
    #     for k, v in i.items():
    #         if k not in album_lst_new:
    #             album_lst_new.append({k:v})

    # for view in album_lst_new:
    #     print(view)
